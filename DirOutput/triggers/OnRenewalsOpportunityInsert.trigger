trigger OnRenewalsOpportunityInsert on Opportunity (before insert)
{
    ...
    for(Territory2 t : [SELECT id,DeveloperName,ParentTerritory2.id, ParentTerritory2.Name, ParentTerritory2.DeveloperName FROM Territory2 where Territory2Type.DeveloperName ='Renwals_Countries']){
    ...
    for(account ac :[select id,name,billingcountrycode from account]){
    ...
    for(Opportunity o :Trigger.New){
        ...
        if(o.Type == 'Perpetual Renewal'){ // only perpetual renewals opportunities will take part in this.
            
            o.Territory2Id = countryIdWithRegionId.get(countryNameWithId.get('RN_'+accountIdWithAccount.get(o.AccountId).billingCountryCode));
